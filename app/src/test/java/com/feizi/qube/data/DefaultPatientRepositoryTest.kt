...
/**
 * Unit tests for [DefaultPatientRepository].
 */
@OptIn(ExperimentalCoroutinesApi::class) // TODO: Remove when stable
class DefaultPatientRepositoryTest {

    @Test
    fun patients_newItemSaved_itemIsReturned() = runTest {
        val repository = DefaultPatientRepository(FakePatientDao())

        repository.add("Repository")

        assertEquals(repository.patients.first().size, 1)
    }
...
private class FakePatientDao : PatientDao {
    ...
    override suspend fun insertPatient(item: Patient) {
        data.add(0, item)
    }
}
